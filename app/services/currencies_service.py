...
class CurrenciesService:
    ...
    async def get_all_currencies(self) -> list[Currencies]:
        """
    ...
    async def _get_currencies_from_api(self) -> list[Currencies]:
        ...
        # Convert to Currencies objects
        return [Currencies(**currency_dict) for currency_dict in combined_currencies.values()]

    async def _get_currencies_from_db(self) -> list[Currencies]:
        ...
        try:
            ...
            logger.info(f"Retrieved {len(currencies)} currencies from database as fallback")
            return currencies
        except Exception as e:
            logger.error(f"Failed to retrieve currencies from database: {e}")
            raise

    async def _sync_currencies_to_db(self, currencies_data: list[Currencies]) -> None:
