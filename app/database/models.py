...
from sqlalchemy import BigInteger, CHAR, DateTime, ForeignKeyConstraint, Identity, Index, Integer, PrimaryKeyConstraint, String, Text, UniqueConstraint, Uuid, text
...
class Dyes(Base):
    ...
    color: Mapped[str] = mapped_column(CHAR(14), nullable=False, comment='Color in rgb format like "[RRR,GGG,BBB]" being RRR GGG and BBB parseable integer numbers')
...
class Rarities(Base):
    ...
    color: Mapped[str] = mapped_column(CHAR(14), nullable=False, comment='The representing color tied to the rarity. For example, ascended gear is pink')
...
class GameAccounts(Base):
    ...
    __table_args__ = (
        ForeignKeyConstraint(['world_id'], ['schema_tyriavault.worlds.id'], ondelete='SET NULL', onupdate='CASCADE', name='fk_game_accounts_worlds'),
        PrimaryKeyConstraint('uuid', name='game_accounts_pkey'),
        {'comment': 'Table holding info about the game account of GW2',
     'schema': 'schema_tyriavault'}
    )

    uuid: Mapped[uuid.UUID] = mapped_column(Uuid, primary_key=True, comment='Game account UUID -> Given always by GW2 API')
    account_name: Mapped[str] = mapped_column(String(80), nullable=False, comment='The name of the game account')
    creation_date: Mapped[datetime.datetime] = mapped_column(DateTime(True), nullable=False, server_default=text('CURRENT_TIMESTAMP'), comment='Creation date of this account')
    fractal_level: Mapped[int] = mapped_column(Integer, nullable=False, server_default=text('1'), comment='The fractal level of the account. Usually a number between 1 and 100')
    last_modified: Mapped[datetime.datetime] = mapped_column(DateTime(True), nullable=False, server_default=text('CURRENT_TIMESTAMP'), comment='When was this account last time modified (as perceived by the API) ?')
    last_fetched: Mapped[datetime.datetime] = mapped_column(DateTime(True), nullable=False, server_default=text('CURRENT_TIMESTAMP'), comment='Last time the data of this item was fetched from the API')
    world_id: Mapped[Optional[int]] = mapped_column(Integer, server_default=text('0'), comment='Id referencing the world where the account is from')
    content_access: Mapped[Optional[dict]] = mapped_column(JSONB, comment='The flags assigned to your account telling you which expansions you own.')
...
class ItemsCache(Base):
    ...
    vendor_value: Mapped[Optional[int]] = mapped_column(Integer, server_default=text('0'), comment='The value in coins when selling to a vendor. (Can be non-zero even when the item has the NoSell flag.)')
...
class ApiKeys(Base):
    ...
    __table_args__ = (
        ForeignKeyConstraint(['game_account_uuid'], ['schema_tyriavault.game_accounts.uuid'], ondelete='SET NULL', onupdate='CASCADE', name='fk_api_keys_game_accounts'),
...
class Bank(Base):
    ...
    __table_args__ = (
        ...
        ForeignKeyConstraint(['game_account_uuid'], ['schema_tyriavault.game_accounts.uuid'], ondelete='CASCADE', onupdate='CASCADE', name='fk_bank_game_accounts'),
        ForeignKeyConstraint(['item_id'], ['schema_tyriavault.items_cache.id'], ondelete='SET NULL', onupdate='CASCADE', name='fk_bank_items_cache'),
    ...
    id: Mapped[int] = mapped_column(BigInteger, Identity(start=1, increment=1, minvalue=1, maxvalue=9223372036854775807, cycle=False, cache=1), primary_key=True, comment='Id autogenerated by TyrianAccount')
    game_account_uuid: Mapped[uuid.UUID] = mapped_column(Uuid, nullable=False, comment='The uuid of the game account holding in its bank this item')
    ...
    charges: Mapped[Optional[int]] = mapped_column(Integer, comment='Remaining charges of this item if there are any (ie a recyling kit). Can be NULL')
...
class Characters(Base):
    ...
    __table_args__ = (
        ForeignKeyConstraint(['game_account_uuid'], ['schema_tyriavault.game_accounts.uuid'], ondelete='CASCADE', onupdate='CASCADE', name='fk_characters_game_accounts'),
    ...
    game_account_uuid: Mapped[Optional[uuid.UUID]] = mapped_column(Uuid, comment='uuId referencing the game_account who own this character')
...
class Emotes(Base):
    ...
    __table_args__ = (
        ForeignKeyConstraint(['unlocking_item_id'], ['schema_tyriavault.items_cache.id'], ondelete='SET NULL', onupdate='CASCADE', name='fk_emotes_items_cache'),
    ...
    id: Mapped[int] = mapped_column(Integer, Identity(start=1, increment=1, minvalue=1, maxvalue=2147483647, cycle=False, cache=1), primary_key=True, comment='Id of the emote. Assigned by TyriaAccount')
...
class UnlockedDyes(Base):
    ...
    __table_args__ = (
        ...
        ForeignKeyConstraint(['game_account_uuid'], ['schema_tyriavault.game_accounts.uuid'], ondelete='CASCADE', onupdate='CASCADE', name='fk_unlocked_dyes_game_accounts'),
    ...
    game_account_uuid: Mapped[Optional[uuid.UUID]] = mapped_column(Uuid, comment='The game account having unlocked the dye')
...
class Wallet(Base):
    ...
    __table_args__ = (
        ...
        ForeignKeyConstraint(['game_account_uuid'], ['schema_tyriavault.game_accounts.uuid'], ondelete='CASCADE', onupdate='CASCADE', name='fk_wallet_game_accounts'),
    ...
    id: Mapped[int] = mapped_column(Integer, Identity(start=1, increment=1, minvalue=1, maxvalue=2147483647, cycle=False, cache=1), primary_key=True, comment='Id of the tuple. Generated by TyriaAccount')
    currency_id: Mapped[int] = mapped_column(Integer, nullable=False, server_default=text('0'), comment='The Id of referencing the currency holded')
    game_account_uuid: Mapped[uuid.UUID] = mapped_column(Uuid, nullable=False, comment='The uuid of the game account owning the currency')
...
class WalletHistory(Base):
    ...
    __table_args__ = (
        ForeignKeyConstraint(['currency_id'], ['schema_tyriavault.currencies.id'], ondelete='CASCADE', onupdate='CASCADE', name='fk_wallet_history_currencies'),
        ForeignKeyConstraint(['game_account_uuid'], ['schema_tyriavault.game_accounts.uuid'], ondelete='CASCADE', onupdate='CASCADE', name='fk_wallet_history_game_accounts'),
    ...
    id: Mapped[int] = mapped_column(BigInteger, Identity(always=True, start=1, increment=1, minvalue=1, maxvalue=9223372036854775807, cycle=False, cache=1), primary_key=True, comment='The id. Autogenerated by TyriaVault')
    currency_id: Mapped[int] = mapped_column(Integer, nullable=False, comment='The Id of referencing the currency holded')
    snapshot_time: Mapped[datetime.datetime] = mapped_column(DateTime(True), nullable=False, server_default=text('CURRENT_TIMESTAMP'), comment='The relevant timestamp for this tuple')
    game_account_uuid: Mapped[Optional[uuid.UUID]] = mapped_column(Uuid, comment='The uuid of the game account owning the currency')
...
class UnlockedEmotes(Base):
    ...
    __table_args__ = (
        ForeignKeyConstraint(['emote_id'], ['schema_tyriavault.emotes.id'], ondelete='CASCADE', onupdate='CASCADE', name='fk_unlocked_emotes_emotes'),
        ForeignKeyConstraint(['game_account_uuid'], ['schema_tyriavault.game_accounts.uuid'], ondelete='CASCADE', onupdate='CASCADE', name='fk_unlocked_emotes_game_accounts'),
    ...
    id: Mapped[int] = mapped_column(BigInteger, Identity(start=1, increment=1, minvalue=1, maxvalue=9223372036854775807, cycle=False, cache=1), primary_key=True, comment='The id of the unlocked emote. Autogenerated by TyriaAccount')
    game_account_uuid: Mapped[uuid.UUID] = mapped_column(Uuid, nullable=False, comment='uuId referencing the game_account who unlocked this emote')
...
class UnlockedMinis(Base):
    ...
    __table_args__ = (
        ...
        PrimaryKeyConstraint('id', name='pk_unlocked_minis'),
        UniqueConstraint('mini_id', 'game_account_uuid', name='unq_unlocked_minis'),
    ...
    mini_id: Mapped[int] = mapped_column(Integer, nullable=False)
