...
async def seed_data(db: AsyncSession):
    ...
    # Seed Races
    logging.info("Seeding Races...")
    for race_data in RACES_DATA:
        result = await db.execute(select(Races).filter_by(name_en=race_data["id"]))
    ...
    # Seed Professions
    logging.info("Seeding Professions...")
    for prof_data in PROFESSIONS_DATA:
        result = await db.execute(select(Professions).filter_by(name_en=prof_data["id"]))
