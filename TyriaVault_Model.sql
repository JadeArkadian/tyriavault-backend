...
CREATE  TABLE schema_tyriavault.unlocked_dyes ( 
	...
	unlock_date          date DEFAULT CURRENT_TIMESTAMP   ,
	CONSTRAINT pk_unlocked_dyes PRIMARY KEY ( game_account_uuid, dye_id ),
...
CREATE INDEX idx_unlocked_dyes ON schema_tyriavault.unlocked_dyes  ( game_account_uuid );

CREATE  TABLE schema_tyriavault.unlocked_minis ( 
	mini_id              integer  NOT NULL  ,
	game_account_uuid    uuid  NOT NULL  ,
	unlock_date          date DEFAULT CURRENT_TIMESTAMP   ,
	CONSTRAINT pk_unlocked_minis PRIMARY KEY ( mini_id, game_account_uuid ),
...
CREATE INDEX idx_unlocked_minis ON schema_tyriavault.unlocked_minis  ( game_account_uuid );

CREATE  TABLE schema_tyriavault.wallet ( 
	currency_id          integer DEFAULT 0 NOT NULL  ,
	game_account_uuid    uuid  NOT NULL  ,
	amount               integer DEFAULT 0   ,
	CONSTRAINT pk_wallet PRIMARY KEY ( currency_id, game_account_uuid ),
...
CREATE INDEX idx_wallet ON schema_tyriavault.wallet  ( game_account_uuid );
...
CREATE  TABLE schema_tyriavault.wallet_history ( 
	...
	CONSTRAINT pk_wallet_history PRIMARY KEY ( currency_id, game_account_uuid, snapshot_time ),
...
CREATE  TABLE schema_tyriavault.unlocked_emotes ( 
	...
	unlock_date          timestamptz DEFAULT CURRENT_TIMESTAMP   ,
	CONSTRAINT pk_unlocked_emotes PRIMARY KEY ( game_account_uuid, emote_id ),
...
CREATE INDEX idx_unlocked_emotes ON schema_tyriavault.unlocked_emotes  ( game_account_uuid );
...
COMMENT ON COLUMN schema_tyriavault.unlocked_dyes.unlock_date IS 'The unlock date';

COMMENT ON TABLE schema_tyriavault.unlocked_minis IS 'Table listing unlocked miniatures for a given game account';

COMMENT ON COLUMN schema_tyriavault.unlocked_minis.unlock_date IS 'The unlock date';
...
COMMENT ON COLUMN schema_tyriavault.unlocked_emotes.unlock_date IS 'The unlock date';
